...
async fn menu(data: &PathBuf) -> Result<()> {
    loop {
        let c = load(data)?;
        let active = systemctl_state("is-active").await;
        let enabled = systemctl_state("is-enabled").await;
        println!("\n\x1b[0;32mRHC Bot 管理界面\x1b[0m\n0. 退出\n1. 更改设置\n2. 查看当前设置\n3. 立即更新本地 UBI 缓存\n4. 重置验证数据\n5. 启动服务\n6. 停止服务\n7. 重启服务\n8. 开启开机自启\n9. 关闭开机自启\n\n服务状态: {active} | 开机自启: {enabled}\n镜像模式: {} | 缓存更新周期: {}", c.image_mode, c.cache_update_interval);
        print!("请选择 [0-9]: ");
        io::stdout().flush()?;
        ...
        match value.trim() {
            ...
            "5" => systemctl("start").await?,
            "6" => systemctl("stop").await?,
            "7" => systemctl("restart").await?,
            "8" => systemctl("enable").await?,
            "9" => systemctl("disable").await?,
            _ => println!("输入无效"),
        }
    }
}
async fn systemctl_state(action: &str) -> String {
    match Command::new("systemctl")
        .args([action, "rhc-bot.service"])
        .output()
        .await
    {
...
async fn systemctl(action: &str) -> Result<()> {
    let output = Command::new("systemctl")
        ...
        .output()
        .await
        .with_context(|| format!("无法执行 systemctl {action}"))?;
    ...
    println!("systemctl {action} rhc-bot.service 执行成功");
    Ok(())
}
#[tokio::main]
    ...
    match cli.command {
        ...
        Some(action) => match action {
            ...
            Action::Status => {
                println!(
                    "服务状态: {}\n开机自启: {}",
                    systemctl_state("is-active").await,
                    systemctl_state("is-enabled").await
                );
                Ok(())
            }
            Action::Reset => {
